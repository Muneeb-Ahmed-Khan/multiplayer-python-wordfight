...
class GameSessionServerInit:
    ...
    def listenForFlag(self, connection):
        ...
        try:
            ...
            try:
                ...
                print("Error in Reading Flag (JSON)---- GameServer")
                self.serverDisposer()
                return False
    ...
    def serverDisposer(self):
        ...
        try:
            ...
            print("Server Disposer ----> Already Sockets Closed")
    ...
    def gracefulEndConnection(self):
        ...
        try:
            ...
            print("Disposer --- ALready clossed the sockets")
        ...
        try:
            ...
            if flag_recv.decode('utf-8')[0] != '{':
                ...
                try:
                    ...
                    print("Error in Reading Flag (JSON)")
                    connection.close()
                    return False

                _SET_FLAG_ = flag
                if flag['UID'] not in ID_TO_CONNECTION_MAP:
                    if queryDbForRegisteredClients(flag['UID'], 'users', 'wordfight'):
                        # _SET_FLAG_ = flag
                        ID_TO_CONNECTION_MAP[flag['UID']] = connection
